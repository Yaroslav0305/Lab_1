...
def encrypt():
    ...
    try:
        ...
        messagebox.showerror("Неправильный ввод", "K должно быть целым числом")
        return

    m = 32 if any(c in 'абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ' for c in keyword) else 26

    if not (0 <= k <= m - 1):
        messagebox.showerror("Неправильный ввод", f"K должно быть в пределах от 0 до {m - 1}")
        return

    if not keyword:
        messagebox.showerror("Неправильный ввод", "Ключевое слово не может быть пустым")
        return
